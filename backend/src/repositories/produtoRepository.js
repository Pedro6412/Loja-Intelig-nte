...
const DEFAULT_CATEGORIA_NAME = 'Geral';

const resolveCategoriaId = async (categoriaId) => {
  if (categoriaId) return categoriaId;

  const categoria = await prisma.categoria.upsert({
    where: { nome: DEFAULT_CATEGORIA_NAME },
    update: {},
    create: {
      nome: DEFAULT_CATEGORIA_NAME,
      descricao: 'Categoria padrao para cadastros rapidos'
    }
  });
...
export const produtoRepository = {
  ...
  create: async (data) => {
    ...
    normalizedData.categoriaId = await resolveCategoriaId(normalizedData.categoriaId);
