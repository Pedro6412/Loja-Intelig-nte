...
export const serializeVenda = (venda) => {
  ...
  const localVenda = pickFirstDefined(venda.localVenda, venda.local_venda) ?? null;
  const enderecoFormatado =
    pickFirstDefined(venda.enderecoFormatado, venda.endereco_formatado) ?? null;
  const dataVenda =
  ...
  return {
    ...
    latitude: toNumber(venda.latitude),
    longitude: toNumber(venda.longitude),
    cidade: venda.cidade ?? null,
    bairro: venda.bairro ?? null,
    regiao: venda.regiao ?? null,
    estado: venda.estado ?? null,
    cep: venda.cep ?? null,
    enderecoFormatado,
