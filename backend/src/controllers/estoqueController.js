...
export const estoqueController = {
  ...
  registrarEntrada: async (req, res, next) => {
    try {
      ...
      const movimentacao = await estoqueService.registrarEntrada(produtoId, quantidade, motivo);
  ...
  registrarSaida: async (req, res, next) => {
    try {
      const { produtoId, quantidade, motivo } = req.body;
      const movimentacao = await estoqueService.registrarSaida(produtoId, quantidade, motivo);
      res.status(201).json(movimentacao);
    } catch (error) {
      next(error);
    }
  ...
  ajustarEstoque: async (req, res, next) => {
    try {
      const { produtoId, quantidade, motivo } = req.body;
      const movimentacao = await estoqueService.ajustarEstoque(produtoId, quantidade, motivo);
      res.status(201).json(movimentacao);
    } catch (error) {
      next(error);
    }
