...
const NotaFiscal = () => {
  ...
  const vendasFiltradas = useMemo(() => {
    ...
    return vendas.filter((venda) => {
      ...
      const cliente = String(venda.cliente?.nome || '').toLowerCase();
      return id.includes(termo) || cliente.includes(termo);
    });
  }, [vendas, busca]);

  const exportarCsv = () => {
    const headers = ['Venda', 'Data', 'Cliente', 'Total', 'Status'];
  ...
  const visualizarNf = (venda) => {
    ...
    const cliente = venda.cliente?.nome || 'Nao informado';
    const mensagem =
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white border border-[#d9d9d9] rounded-xl p-4">
        <input
          ...
          placeholder="Buscar por numero da venda ou cliente"
            ...
            <tbody className="divide-y divide-[#ececec]">
              {vendasFiltradas.map((venda) => (
                <tr key={venda.id} className="text-sm text-slate-700">
                  ...
                  <td className="px-4 py-3">{venda.cliente?.nome || 'Nao informado'}</td>
